...
import { formatCanonicalRef, formatResourceRef, parseResourceRef } from "../src/refs.js";
...
describe("Forgejo references", () => {
  it("round-trips qualified issue and pull references", () => {
    ...
    expect(issue).toEqual({ server: "work", owner: "platform", repo: "api", kind: "issue", index: 184 });
    expect(pull).toEqual({ server: "community", owner: "forge", repo: "runner", kind: "pull", index: 77 });
    expect(formatResourceRef(issue!)).toBe("work:platform/api#184");
    expect(formatCanonicalRef(pull!)).toBe("fj://community/forge/runner/pulls/77");
  });
