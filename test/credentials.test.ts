...
describe("EnvironmentCredentialProvider", () => {
  ...
  it("rejects missing or whitespace-bearing token values without exposing them", async () => {
    const missing = new EnvironmentCredentialProvider(
      "work",
      "FORGEJO_WORK_TOKEN",
      {},
    );
    const invalid = new EnvironmentCredentialProvider(
      "work",
      "FORGEJO_WORK_TOKEN",
      {
    ...
    await expect(missing.getToken()).rejects.toThrow(
      "FORGEJO_WORK_TOKEN is not set",
    );
    await expect(invalid.getToken()).rejects.toSatisfy((error: unknown) => {
      expect(error).toBeInstanceOf(CredentialError);
      expect((error as Error).message).toContain(
        "FORGEJO_WORK_TOKEN is invalid",
      );
...
describe("FgjCredentialProvider", () => {
  it("loads a token once, keeps it out of process arguments, and reloads after clear", async () => {
    ...
    const provider = new FgjCredentialProvider(
      "work",
      "git.example.dev",
      "/workspace",
      exec,
    ...
    ).resolves.toEqual(["secret-token", "secret-token"]);
    ...
    expect(exec.mock.calls.flatMap((call) => call[1])).not.toContain(
      "secret-token",
    );
  ...
  it("isolates a cancelled caller from a shared credential lookup", async () => {
    ...
    const first = provider.getToken(firstController.signal);
    const second = provider.getToken(secondController.signal);
    ...
    expect(exec.mock.calls[0]?.[2].signal?.aborted).toBe(false);
    settle?.({ code: 0, stdout: "shared-token\n", stderr: "" });
