...
const FORGEJO_TOOL_DOMAINS = ["issue", "pull", "review", "actions", "notifications", "search", "dashboard"] as const;
...
export function registerForgejoTools(pi: ExtensionAPI, runtimeProvider: RuntimeProvider): ForgejoToolController {
  ...
  pi.registerTool({
    ...
    promptGuidelines: ["Use forgejo_tools before an unavailable Forgejo operation."],
    parameters: Type.Object({
      domains: Type.Array(StringEnum(FORGEJO_TOOL_DOMAINS), {
        minItems: 1,
        maxItems: MAX_DOMAINS_PER_LOAD,
        uniqueItems: true,
        description: "Activate one to four domains; call again only if the task expands",
      }),
    }),
    async execute(_toolCallId, params) {
      if (params.domains.length > MAX_DOMAINS_PER_LOAD) {
        throw new Error(`forgejo_tools activates at most ${MAX_DOMAINS_PER_LOAD} domains per call; load the next group only when needed`);
      }
      if (typeof pi.getActiveTools !== "function" || typeof pi.setActiveTools !== "function") {
        return toolResult("Dynamic Forgejo tool activation is unavailable in this Pi version; registered tools remain unchanged.", {
      ...
      return toolResult(
        added.length > 0 ? `Enabled Forgejo tools: ${added.join(", ")}` : `Requested Forgejo tools already active: ${selected.join(", ")}`,
