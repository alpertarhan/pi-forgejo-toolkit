...
import { formatCanonicalRef, formatRepoRef, formatResourceRef, parseResourceRef } from "../refs.js";
...
function summarizeItems(label: string, items: DashboardItem[], total: number): string {
  const lines = items.map((item) => `- ${itemRef(item)} ${item.title}`);
  return [`${label}: ${total}`, ...lines].join("\n");
}

export function registerContextTools(pi: ExtensionAPI, runtimeProvider: RuntimeProvider): void {
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      ref: Type.Optional(Type.String({ description: "Forgejo reference to resolve" })),
    }),
    async execute(_toolCallId, params, signal) {
      const runtime = runtimeProvider();
      if (params.action === "current") {
        const repo = runtime.currentRepo();
        const reason = runtime.repoResolution.status === "resolved" ? "repository context is not selected" : runtime.repoResolution.reason;
        const summary = repo ? `Active Forgejo repository: ${formatRepoRef(repo)}` : `No active Forgejo repository: ${reason}`;
        return toolResult(summary, { repo, resolution: runtime.repoResolution, server: runtime.currentServer() });
      }
      if (params.action === "servers") {
        const servers = Object.entries(runtime.config.servers).map(([alias, config]) => ({
          alias,
        ...
        return toolResult(`Configured Forgejo servers: ${servers.map((server) => server.alias).join(", ")}`, servers);
      }
      ...
      if (params.action === "resolve_ref") {
        ...
        return toolResult(`${formatResourceRef(ref)} -> ${formatCanonicalRef(ref)}`, { ref, canonical: formatCanonicalRef(ref) });
      }

      const capabilities = await runtime.capabilities.refresh(signal);
      if (params.action === "whoami") {
        ...
        if (params.server && !identities[params.server]) {
          throw new Error(capabilities.errors[params.server] ?? `unknown Forgejo server '${params.server}'`);
        }
      ...
      const health = Object.fromEntries(
        runtime.clients.aliases().map((alias) => [
          alias,
          capabilities.values[alias] ? { status: "ok", version: capabilities.values[alias]?.version } : { status: "error", error: capabilities.errors[alias] },
        ]),
      );
      return toolResult(
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal) {
      ...
      if (params.action === "get_attention_items") {
        const items = snapshot.attention.filter((item) => !params.server || item.server === params.server).slice(0, limit);
        ...
        params.action === "get_assigned_issues"
          ...
          : params.action === "get_authored_pulls"
            ...
            : params.action === "get_failed_runs"
              ? "failedRuns"
              : "reviewRequests";
      const total = serverValues.reduce((sum, server) => sum + server[collectionKey].total, 0);
      const items = serverValues.flatMap((server) => server[collectionKey].items).slice(0, limit);
      const label =
