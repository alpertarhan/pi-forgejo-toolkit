...
function formatPull(
  ...
  const lines = [
    ...
    `Reviews (${reviews.length}):`,
		reviews.length > 0
			? reviews.map(formatForgejoReview).join("\n\n")
			: "(none)",
  ];
  return lines.join("\n");
}
  ...
  const footer = (truncated: boolean): string =>
    [
      ...
      `Truncated: ${truncated ? "yes" : "no"}`,
			...(truncated
				? [`Recovery: repeat page ${page} with a smaller limit`]
				: []),
    ].join("\n");
...
function reviewTimestamp(review: ForgejoPullReview): number {
	return (
		Date.parse(review.updated_at ?? review.submitted_at ?? "") || review.id
	);
}
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      body: Type.Optional(Type.String()),
			...
			reviewers: Type.Optional(
				Type.Array(Type.String({ minLength: 1 }), {
					description:
						"Usernames to request or remove from pull-request review",
				}),
			),
			reviewer_teams: Type.Optional(
				Type.Array(Type.String({ minLength: 1 }), {
					description:
						"Team names to request or remove from pull-request review",
				}),
			),
			labels: Type.Optional(
				Type.Array(Type.String(), {
					description:
						"Desired pull-request labels; an empty list clears labels",
				}),
			),
      ...
      base: Type.Optional(Type.String({ description: "Target branch" })),
			draft: Type.Optional(
				Type.Boolean({
					description:
						"Prefix a new PR title with the Forgejo default WIP marker",
				}),
			),
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (params.action === "updates") {
				return incrementalConversationUpdates<ForgejoPullRequest>(
					runtime,
					ref,
					{
      ...
      if (params.action === "set_maintainer_edit") {
        ...
        const response = await client.request<ForgejoPullRequest>(pullPath, {
          ...
          body: { allow_maintainer_edit: params.allow_maintainer_edit },
				...
				if (
					response.data.allow_maintainer_edit !== params.allow_maintainer_edit
				) {
					throw new Error(
        ...
        const reviewers = uniqueNames(params.reviewers ?? [], "reviewers");
				const teams = uniqueNames(
					params.reviewer_teams ?? [],
					"reviewer_teams",
				);
        ...
        await client.request(`${pullPath}/requested_reviewers`, {
          ...
          body: { reviewers: changedReviewers, team_reviewers: changedTeams },
				...
				const freshReviewers = new Set(
					fresh.data.requested_reviewers?.map((user) =>
						user.login.toLowerCase(),
					) ?? [],
				);
        ...
        const unverified = [
					...reviewers
						.filter(
							(name) => freshReviewers.has(name.toLowerCase()) !== requesting,
						)
						.map((name) => `user:${name}`),
					...teams
      ...
      if (params.action === "close" || params.action === "reopen") {
        ...
        if (current.data.state === targetState) {
					return toolResult(
						`${reference} is already ${targetState}`,
						current.data,
					);
        }
      ...
      if (params.action === "readiness" || params.action === "merge") {
				const readiness = await loadMergeReadiness(
					client,
					ref,
					pull.data,
					signal,
