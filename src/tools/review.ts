...
function draftSummary(draft: ReviewDraft): string {
  ...
  for (const comment of draft.comments) {
    const position = comment.newPosition ? `new:${comment.newPosition}` : `old:${comment.oldPosition ?? 0}`;
    lines.push(`- ${comment.path}:${position} ${comment.body}`);
  }
  return lines.join("\n");
}

export function registerReviewTool(pi: ExtensionAPI, runtimeProvider: RuntimeProvider): void {
  pi.registerTool({
    name: "forgejo_review",
    ...
    parameters: Type.Object({
      ...
      verdict: Type.Optional(StringEnum(["COMMENT", "APPROVE", "REQUEST_CHANGES"] as const)),
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (params.action === "get" || params.action === "get_comments") {
        if (!params.review_id) throw new Error(`review_id is required for ${params.action}`);
        const commentsPath = `${reviewsPath}/${params.review_id}/comments`;
        if (params.action === "get_comments") {
          const response = await client.request<ForgejoPullReviewComment[]>(commentsPath, requestOptions);
          const text = [
        ...
        const [review, comments] = await Promise.all([
          client.request<ForgejoPullReview>(`${reviewsPath}/${params.review_id}`, requestOptions),
          client.request<ForgejoPullReviewComment[]>(commentsPath, requestOptions),
        ]);
        const text = [
      ...
      if (params.action === "create_draft") {
        if (!params.verdict) throw new Error("verdict is required for create_draft");
      ...
      if (!draft) throw new Error(`no review draft exists for ${reference}; create one first`);
      if (params.action === "add_inline_comment") {
        if (!params.path || !params.body) throw new Error("path and body are required for add_inline_comment");
        if (!params.new_position && !params.old_position) throw new Error("new_position or old_position is required for add_inline_comment");
        const comment = { path: params.path, body: params.body } as ReviewDraft["comments"][number];
        if (params.new_position !== undefined) comment.newPosition = params.new_position;
        if (params.old_position !== undefined) comment.oldPosition = params.old_position;
      ...
      if (params.action === "discard") {
        ...
        return toolResult(`Discarded review draft for ${reference}`, { discarded: true, ref });
      }
      if (!draft.previewedAt) throw new Error(`review draft for ${reference} must be previewed before submit`);

      const currentPull = await client.request<ForgejoPullRequest>(pullPath, requestOptions);
      if (draft.commitId && currentPull.data.head.sha !== draft.commitId) {
        throw new Error(`pull request head changed from ${draft.commitId} to ${currentPull.data.head.sha}; rebuild the review draft`);
      }
      await confirmMutation(
        ctx,
        "Submit Forgejo review",
