...
const ACTION_STATUSES = ["unknown", "waiting", "running", "success", "failure", "cancelled", "skipped", "blocked"] as const;
const FINISHED_ACTION_STATUSES = new Set<ForgejoActionStatus>(["success", "failure", "cancelled", "skipped"]);
...
async function actionFeature(
  ...
  if (!capabilities) {
    await runtime.capabilities.refresh(signal);
    capabilities = runtime.capabilities.get(server);
  }
...
async function requireActionFeature(
  ...
  if (availability === "available" || (!mutate && availability === "unknown")) return;
  const version = runtime.capabilities.get(server)?.version ?? "unknown version";
  if (availability === "unavailable") {
    throw new Error(`Forgejo ${server} (${version}) does not advertise the ${featureLabel(feature)} API`);
  }
  throw new Error(
...
async function artifactOutputPath(
  ...
  if (!isWithin(workspace, lexical)) throw new Error("artifact output_path must stay inside the current workspace");
  const [realWorkspace, realParent] = await Promise.all([realpath(workspace), realpath(dirname(lexical))]);
  if (!isWithin(realWorkspace, realParent)) {
    throw new Error("artifact output_path parent resolves outside the current workspace");
  }
...
async function writeArchiveAtomic(destination: string, bytes: Uint8Array, overwrite: boolean): Promise<void> {
  ...
  try {
    if (overwrite) {
      ...
      try {
        ...
        if (errorCode(error) === "EEXIST") throw new Error(`artifact output already exists: ${destination}`);
        throw error;
      }
...
export function registerActionsTool(pi: ExtensionAPI, runtimeProvider: RuntimeProvider): void {
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      ...repoTargetProperties,
      run_id: Type.Optional(Type.Integer({ minimum: 1, description: "Workflow run database ID returned by list" })),
      job_id: Type.Optional(Type.Integer({ minimum: 1, description: "Workflow job ID returned by jobs" })),
      attempt: Type.Optional(Type.Integer({ minimum: 1, description: "Historical job attempt; omit for latest" })),
      artifact_id: Type.Optional(Type.Integer({ minimum: 1, description: "Artifact ID returned by artifacts" })),
      artifact_name: Type.Optional(Type.String({ minLength: 1, description: "Exact artifact-name filter for artifacts" })),
      status: Type.Optional(StringEnum(ACTION_STATUSES)),
      event: Type.Optional(Type.String({ description: "Trigger event such as push or pull_request" })),
      git_ref: Type.Optional(Type.String({ minLength: 1, description: "Git reference for filtering or workflow dispatch" })),
      head_sha: Type.Optional(Type.String()),
      workflow_id: Type.Optional(Type.String({ minLength: 1, description: "Workflow filename" })),
      inputs: Type.Optional(Type.Record(Type.String(), Type.String(), { description: "Workflow-dispatch input values" })),
      output_path: Type.Optional(Type.String({ minLength: 1, description: "Artifact ZIP destination relative to the current workspace" })),
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (params.action === "dispatch") {
        await requireActionFeature(runtime, repo.server, "actionsDispatch", true, signal);
        ...
        return toolResult(summary, { workflow, ref, inputNames: Object.keys(inputs), run });
      }

      if (params.action === "cancel" || params.action === "rerun") {
        const runId = requireId(params.run_id, "run_id");
        const feature = params.action === "cancel" ? "actionsCancel" : "actionsRerun";
        ...
        if (params.action === "cancel" && FINISHED_ACTION_STATUSES.has(current.status)) {
          return toolResult(`Actions run ${current.index_in_repo} (id ${runId}) is already finished [${current.status}]`, current);
        }
        if (params.action === "rerun" && !FINISHED_ACTION_STATUSES.has(current.status)) {
          throw new Error(`Actions run ${current.index_in_repo} (id ${runId}) is not finished [${current.status}]`);
        }
        await confirmMutation(
          ctx,
          params.action === "cancel" ? "Cancel Forgejo Actions run" : "Rerun Forgejo Actions run",
          [
            `Server: ${repo.server}`,
            `Repository: ${repo.owner}/${repo.repo}`,
      ...
      if (params.action === "artifacts" || params.action === "artifact" || params.action === "download_artifact") {
        await requireActionFeature(runtime, repo.server, "actionsArtifacts", false, signal);
        if (params.action === "artifacts") {
          const filters: ActionArtifactFilters = {
            page: params.page ?? 1,
            limit: positiveLimit(params.limit, 20, 50),
          };
          if (params.run_id !== undefined) filters.runId = requireId(params.run_id, "run_id");
          if (params.artifact_name !== undefined) filters.name = params.artifact_name;
          const page = await listActionArtifacts(client, repo, filters, signal);
          return toolResult(
            [`Artifacts for ${repo.server}:${repo.owner}/${repo.repo}: ${page.total}`, ...page.artifacts.map(formatArtifact)].join("\n"),
            page,
          );
        }
        ...
        if (params.action === "artifact") return toolResult(formatArtifact(artifact).slice(2), artifact);
        if (artifact.expired) throw new Error(`artifact ${artifact.id} '${artifact.name}' has expired`);
        const maximum = params.max_download_bytes ?? DEFAULT_ARTIFACT_LIMIT;
        if (artifact.size_in_bytes > maximum) {
          throw new Error(`artifact ${artifact.id} metadata size ${artifact.size_in_bytes} exceeds the ${maximum} byte limit`);
        }
        const destination = await artifactOutputPath(runtime.cwd, params.output_path, artifact);
        const exists = await pathExists(destination);
        if (exists && !params.overwrite) throw new Error(`artifact output already exists: ${destination}`);
        if (exists) {
          await confirmMutation(
            ctx,
            "Overwrite Forgejo artifact",
            [
              `Server: ${repo.server}`,
              `Repository: ${repo.owner}/${repo.repo}`,
        ...
        const bytes = await downloadActionArtifact(client, repo, artifactId, maximum, signal);
        await writeArchiveAtomic(destination, bytes, Boolean(params.overwrite));
        return toolResult(`Downloaded artifact ${artifact.id} '${artifact.name}' to ${destination} (${bytes.byteLength} bytes)`, {
          artifact,
          path: destination,
      ...
      await requireActionFeature(runtime, repo.server, "actionsRuns", false, signal);
      if (params.action === "list") {
        const filters: ActionRunFilters = { limit: positiveLimit(params.limit, 20, 50) };
        if (params.page !== undefined) filters.page = params.page;
        if (params.event !== undefined) filters.event = params.event;
        if (params.status !== undefined) filters.status = params.status as ForgejoActionStatus;
        if (params.head_sha !== undefined) filters.headSha = params.head_sha;
        if (params.git_ref !== undefined) filters.ref = params.git_ref;
        if (params.workflow_id !== undefined) filters.workflowId = params.workflow_id;
        ...
        return toolResult(
          [`Actions runs for ${repo.server}:${repo.owner}/${repo.repo}: ${page.total}`, ...lines].join("\n"),
          page,
        );
      }
      ...
      if (params.action === "jobs") {
        ...
        const lines = jobs.map((job) => `- job ${job.id} ${job.name} [${job.status}] attempt ${job.attempt}`);
        return toolResult([`Jobs for Actions run id ${runId}: ${jobs.length}`, ...lines].join("\n"), jobs);
      }
