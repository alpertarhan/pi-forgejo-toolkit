...
async function requireActionFeature(
  ...
  const availability = await actionFeature(runtime, server, feature, signal);
	if (availability === "available" || (!mutate && availability === "unknown"))
		return;
	const version =
		runtime.capabilities.get(server)?.version ?? "unknown version";
      ...
      head_sha: Type.Optional(Type.String()),
			...
			output_path: Type.Optional(
				Type.String({
					minLength: 1,
					description:
						"Artifact ZIP destination relative to the current workspace",
				}),
			),
      ...
      limit: Type.Optional(Type.Integer({ minimum: 1, maximum: 50 })),
			...
			max_download_bytes: Type.Optional(
				Type.Integer({
					...
					description:
						"Maximum artifact metadata size and downloaded ZIP bytes",
				}),
			),
    }),
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (params.action === "cancel" || params.action === "rerun") {
        ...
        const artifactId = requireId(params.artifact_id, "artifact_id");
				const artifact = await getActionArtifact(
					client,
					repo,
					artifactId,
					signal,
				...
				if (artifact.expired)
					throw new Error(
						`artifact ${artifact.id} '${artifact.name}' has expired`,
					);
