...
const SHORT_REF = new RegExp(`^(${SEGMENT_PATTERN}):(${SEGMENT_PATTERN})/(${SEGMENT_PATTERN})([#!])(\\d+)$`);
...
export function parseResourceRef(value: string): ResourceRef | undefined {
  ...
  if (shortMatch) {
    ...
    if (Number(rawIndex) < 1) return undefined;
    return {
      server,
      owner,
      repo,
  ...
  if (!owner || !repo || !rawKind || !rawIndex || !/^\d+$/.test(rawIndex)) return undefined;
  if (Number(rawIndex) < 1) return undefined;
...
export function resourceWebUrl(ref: ResourceRef, server: ForgejoServerConfig): string {
  const kind = ref.kind === "issue" ? "issues" : "pulls";
