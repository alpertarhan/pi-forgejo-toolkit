...
import type { ForgejoConfig, GitRemote, RepoResolution, ResolvedRemote } from "./types.js";
...
function parseRemoteUrl(value: string): ParsedRemoteUrl | undefined {
  ...
  if (scpMatch && !value.includes("://")) {
    ...
    return { host: host.toLowerCase(), path, sshHost: host.toLowerCase() };
  }
...
function repoSegments(path: string, baseUrl: string): { owner: string; repo: string } | undefined {
...
export function parseGitRemotes(output: string): GitRemote[] {
  ...
  for (const line of output.split("\n")) {
    ...
    if (!name || !url || (direction !== "fetch" && direction !== "push")) continue;
...
export async function discoverSshHostAliases(
  ...
  for (const remote of remotes) {
    ...
    const destination: { host: string; port?: string } = { host: parsed.sshHost };
  ...
  await Promise.all(
    [...destinations.values()].map(async (destination) => {
      ...
      const options = signal === undefined ? { cwd, timeout: 5_000 } : { cwd, timeout: 5_000, signal };
      ...
      if (!hostname) return;
      aliases[destination.host] = [...new Set([hostname, port ? `${hostname}:${port}` : hostname])];
    }),
  );
  return aliases;
}

export function matchForgejoRemotes(remotes: GitRemote[], config: ForgejoConfig, sshAliases: SshHostAliases = {}): ResolvedRemote[] {
  ...
  for (const remote of remotes) {
    ...
    for (const [server, serverConfig] of Object.entries(config.servers)) {
      ...
      const configuredHosts = [baseUrl.host, baseUrl.hostname, ...serverConfig.remoteHosts].map((host) => host.toLowerCase());
      if (!candidateHosts.some((host) => configuredHosts.includes(host))) continue;
      const repo = repoSegments(parsed.path, serverConfig.baseUrl);
      if (!repo) continue;
      matches.push({
        server,
        owner: repo.owner,
...
export function resolveRepoFromRemotes(remotes: GitRemote[], config: ForgejoConfig, sshAliases: SshHostAliases = {}): RepoResolution {
  const matches = matchForgejoRemotes(remotes, config, sshAliases);
  if (matches.length === 0) {
    return { status: "none", reason: "no Git remote matches a configured Forgejo server" };
  }
  ...
  for (const match of matches) {
    ...
    if (!current || (current.remote !== "origin" && match.remote === "origin")) unique.set(key, match);
  }
  ...
  if (repositories.length !== 1) {
    return {
      ...
      reason: "multiple configured Forgejo repositories match local Git remotes; select one explicitly",
    };
  }
  const [match] = repositories;
  if (!match) return { status: "none", reason: "no Forgejo repository resolved" };
  return {
    status: "resolved",
...
export async function resolveRepository(exec: CommandExecutor, cwd: string, config: ForgejoConfig): Promise<RepoResolution> {
  const result = await exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
  if (result.code !== 0) {
    return {
      status: "none",
      reason: result.stderr.trim() || "current directory is not a Git repository",
    };
  }
